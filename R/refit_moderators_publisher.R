# Current source-audited joint publication/publisher-access model.
# Run from project root: Rscript R/refit_moderators_publisher.R
...
cat('Fitting audited publisher access:',nrow(d),'paper-years;',n_distinct(d$doi_o),'originals\n')
...
saveRDS(d,'data/primary_Scopus_citation_publisher_access.rds')
saveRDS(fit,'models/reg_model_Scopus_citation_publisher_access.rds')
saveRDS(cr,'models/reg_model_Scopus_citation_publisher_access_CR2.rds')
...
write.csv(co,'docs/moderator_coefficients_publisher_access.csv',row.names=FALSE)
paper <- distinct(d,doi_o,.keep_all=TRUE)
write.csv(count(paper,publisher_access_group,is_oa,unpaywall_coverage,multi_original_f),
  'docs/moderator_publisher_access_crosstab.csv',row.names=FALSE)
write.csv(select(paper,doi_o,doi_r,type_r,is_oa,publisher_access_group,
  unpaywall_coverage,publisher_oa,publisher_access_basis,recovered_doi_r,n_originals_for_replication,multi_original_f),
  'docs/moderator_publisher_access_coding_audit.csv',row.names=FALSE)
print(count(paper,publisher_access_group))
...
writeLines(c(paste('Rows:',nrow(d)),paste('Originals:',n_distinct(d$doi_o)),
  ...
  'docs/moderator_refit_publisher_access.txt')
