# Preregistered additive sensitivity model: publication strategy and any-copy
# access as separate terms. Run from project root: Rscript R/refit_moderators_corrected.R
...
formula <- moderator_primary_formula(d)
X <- model.matrix(formula,d)
stopifnot(!anyNA(X),qr(X)$rank==ncol(X),
  max(abs(rowSums(d[grep('^subj_',names(d),value=TRUE)])-1))<1e-10)
cat('Fitting corrected additive model:',nrow(d),'paper-years;',n_distinct(d$doi_o),'originals\n')
...
saveRDS(d,'data/primary_Scopus_citation_corrected.rds')
saveRDS(fit,'models/reg_model_Scopus_citation_corrected.rds')
saveRDS(cr,'models/reg_model_Scopus_citation_corrected_CR2.rds')
...
write.csv(co,'docs/moderator_coefficients_corrected.csv',row.names=FALSE)
...
writeLines(c(paste('Rows:',nrow(d)),paste('Originals:',n_distinct(d$doi_o)),
  ...
  'docs/moderator_refit_corrected.txt')
